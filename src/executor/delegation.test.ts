...
describe('createDelegationManager', () => {
  ...
  describe('detachBridgeForCompaction (#204)', () => {
    it('preserves delegate session (does not call delegate.close or sub.close)', async () => {
      // Track whether the DelegateWiring close was invoked.
      let delegateClosed = false;
      const { manager } = wire((role, id, sub) => {
        const executor = runExecutor(sub, id, async (m) => ({ type: 'result', payload: `done: ${String(m.payload)}` }), role);
        return {
          close() {
            delegateClosed = true;
            executor.close();
      ...
      manager.detachBridgeForCompaction(id);

      // DelegateWiring.close() was NOT called — the delegate session survives.
      expect(delegateClosed).toBe(false);
    });

    it('stops old bridge from forwarding delegate replies to the spawner channel', async () => {
      const { manager, orchestratorInbox, subChannels } = wire(resultDelegate);

      const id = manager.spawn('executor', { type: 'text', payload: 'task' });
      await flush();
      expect(orchestratorInbox).toHaveLength(1); // initial report bridged

      manager.detachBridgeForCompaction(id);

      // The driver is closed; a probe on the sub-channel can send but it won't
      // reach the spawner (the driver no longer listens on the sub-channel).
      const sub = subChannels.get(id)!;
      const probe = sub.join('probe', 'planner', () => {});
      probe.send({ type: 'result', payload: 'late hand-off' });
      await flush();

      expect(orchestratorInbox).toHaveLength(1); // no double-delivery
    });

    it('is idempotent for an unknown or already-detached id', () => {
      const { manager } = wire();
      expect(() => manager.detachBridgeForCompaction('never-spawned')).not.toThrow();
    });

    it('after detach, rewire re-establishes the bridge for the same id', async () => {
      const { manager, orchestratorInbox } = wire(resultDelegate);

      const id = manager.spawn('executor', { type: 'text', payload: 'original' });
      await flush();
      expect(orchestratorInbox).toHaveLength(1);

      manager.detachBridgeForCompaction(id);

      // Rewire with a fresh sub-channel (simulates the new generation's provisioning).
      const newSub = new InMemoryChannel();
      const agentSeat = `${id}-executor`;
      const agent = runExecutor(newSub, agentSeat, async (m) => ({ type: 'result', payload: `rewired: ${m.payload}` }), 'executor');
      manager.rewire('executor', id, newSub, { reportFrom: agentSeat, close: agent.close });

      manager.send(id, { type: 'text', payload: 'follow-up' });
      await flush();

      expect(orchestratorInbox).toHaveLength(2);
      expect(orchestratorInbox[1]).toMatchObject({ from: id, type: 'result', payload: 'rewired: follow-up' });
    });
  });
