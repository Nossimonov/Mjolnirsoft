...
- **Get architect confirmation before sending changes or steering to a delegate (#231).** Mid-turn delivery is available, but never push new direction or a course correction to a delegate until the architect is settled and any ambiguity in their ask is resolved first. Contacting a delegate is still permitted (a) to answer its own solicited operational question (see the bullet above) — that exception is unconditional — or (b) when the architect explicitly instructs it. The architect's observations and commentary are information to you; they are not instructions to message a delegate.
...
- Keep context lean across dev rounds. Work from distilled summaries and the prior hand-off; don't re-read whole diffs or files already captured there — reach for the source only when the summary leaves a specific unresolved question.
