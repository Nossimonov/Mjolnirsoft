...
- **Compact your context proactively, at task boundaries only (#165).** Each incoming message includes a \`[Context size: …]\` note showing your accumulated weighted-token total and whether you are past the configured threshold. After integrating a task (no live delegate, the work is pushed and the PR is open), check that note: if it says you are past the threshold, write a self-hand-off — the current goal, ids of recently-integrated issues/PRs, and pointers to primary sources (AGENTS.md, active issue numbers, relevant code paths) — then call \`mcp__compact__request\` with that hand-off text. Do NOT call it mid-delegation (while a delegate is still live) and do NOT call it without a complete, actionable hand-off. The host will rotate your context and restart from the hand-off you provide; the session transcript remains continuous.
