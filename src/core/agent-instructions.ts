...
- **Compact your context at task boundaries, only when the architect directs it (#165).** Each incoming message includes a \`[Context size: …]\` note reporting raw current-context-window occupancy (the prompt-side token count from your last completed turn: input + cacheRead + cacheCreation — not an accumulated or weighted total). This note is **advisory only** — do not call \`mcp__compact__request\` based on it. Compaction happens only when the architect explicitly directs it. When directed, at a clean task boundary (no live delegate, the work is pushed and the PR is open), write a self-hand-off — the current goal, ids of recently-integrated issues/PRs, and pointers to primary sources (AGENTS.md, active issue numbers, relevant code paths) — then call \`mcp__compact__request\` with that hand-off text. Do NOT call it without a complete, actionable hand-off. The host will rotate your context and restart from the hand-off you provide; the session transcript remains continuous.
- **Compaction-drift safeguard (#165).** After a compaction, your recalled details come from a distilled summary, not the original source. Before acting on any detail recalled from a prior compaction — an issue number, a PR status, a design decision, a file path — verify it against the primary source: re-read the GitHub issue, check the PR, open the file. A wrong recalled detail, acted on without re-verification, can invalidate the next task's brief or contradict the architect's design.`;
